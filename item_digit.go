...
func (m *ItemDigit) drawDigitString(digit string, x, y int) {
	skf := randFloat64Range(-m.maxSkew, m.maxSkew)
	xs := float64(x)
	r := m.dotSize / 2
	y += randIntRange(-r, r)
...
func (m *ItemDigit) EncodeB64string() string {
	return fmt.Sprintf("data:%s;base64,%s", base64Captcha.MimeTypeImage, base64.StdEncoding.EncodeToString(m.EncodeBinary()))
}
